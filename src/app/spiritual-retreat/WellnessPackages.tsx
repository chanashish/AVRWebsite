...
const Card: React.FC<PackageType> = ({ img, caption, title, desc }) => {
  return (
    <div className="flex flex-col gap-2 shadow-md max-w-sm mx-auto text-left transition-transform hover:scale-105 bg-white">
      <img src={img} alt={caption} className="w-full h-[200px] object-cover" />
      <div className='p-4'>
...
const WellnessPackages: React.FC = () => {
  return (
    <div className="w-full text-center max-w-[1224px] mx-auto py-12">
      <div className="flex flex-col max-w-[912px] mx-auto gap-[32px] max-md:px-3">
      ...
      <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-[24px] mt-12 px-4 mb-[40px]">
        {packages.map((item) => (
